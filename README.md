...
| 🗳️ 予想大会 | お題と選択肢（2〜10個）を立てて、みんなで賭ける。**集まった全額を正解者で山分け**。人気のない選択肢を当てるほど儲かる。出題者は賞金の上乗せ・中止もできる |
...
**🎮 あそぶ → 🗳️ 予想大会 → 🆕 お題を立てる** から、お題・選択肢（改行区切りで2〜10個）・締切・参加費を入れます。参加費を空欄にすると、各自が好きな額を賭けられます。

```
① お題を立てる → チャンネルに投稿される
② みんなが選択肢のボタンを押して賭ける（1人1つ、変更不可）
③ 締切がくると自動で締まる（出題者が手で締めることも可）
④ 出題者が「✅ 正解を決める」で正解を選ぶ
⑤ 集まった全額を正解者で山分け
```

山分けは**賭けた額に比例**します。参加費を決めた場合は全員同じ額なので等分になります。
