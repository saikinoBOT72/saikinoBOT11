...
| 🔥 連日ボーナス | 毎日続けて報告すると連続日数が伸び、「N日連続でMコイン」のボーナスが出ます。日数と金額は管理者が自由に決められます。1日空くとやり直し |
| 🏅 称号 | 条件を満たすと自動で贈られます（例:「筋トレ王 = 筋トレ100回」「鉄の意志 = 30日連続」）。条件・名前・絵文字・ボーナスはすべて管理者が作れます |
...
**⚙️ 管理 → 🔥 連日ボーナス → ➕ 追加・変更** で「何日連続で」「何コイン」を入れるだけです。3日・7日・30日のように段階を作ると続きやすくなります。

**⚙️ 管理 → 🏅 称号 → ➕ 新しく作る** では、まず条件の種類を選びます。

| 条件 | 例 |
| --- | --- |
| 報告の合計回数 | 100回で「継続の鬼」 |
| 特定アクションの回数 | 筋トレ50回で「筋トレ王」 |
| 連続報告日数 | 30日で「鉄の意志」 |
| 所持金 | 10000で「富豪」 |

条件を満たした瞬間に自動で贈られ、チャンネルにも告知されます。獲得状況は各自が **お財布 → 🏅 称号・連続記録** で確認できます。
...
```bash
npm install
npm test    # Cloudflare にも Discord にも繋がずに検証（112件）
...
src/
  ...
  menu/               画面
    ...
    titles.js         称号と連続記録の確認
    admin.js          管理メニュー
  lib/                中身のロジック（通貨・報告・連日ボーナス・称号・ショップ・じゃんけん・スロット）
