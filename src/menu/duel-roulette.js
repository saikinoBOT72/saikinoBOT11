...
export function inviteFields({ em }) {
  return [
...
export async function start(ctx, { duel, settings, em }) {
  ...
  return board(started, state, settings, em, `先攻は <@${userIdOf(duel, duel.turn)}> に決まりました。`);
}
...
export async function handle(ix, ctx, { duel, role, state, action }) {
  ...
  if (hit) {
    ...
    ctx.animate(ix, [
      { after: 900, payload: pullFrame(duel, state, em, ix.userId, '…') },
      { after: 1100, payload: resultPayload(duel, next, settings, em, winner, pot) },
    ]);
    return update(pullFrame(duel, state, em, ix.userId, ''));
  }
  ...
  ctx.animate(ix, [
    { after: 900, payload: pullFrame(duel, state, em, ix.userId, '…') },
    { after: 1000, payload: board(moved, next, settings, em, `カチッ…　<@${ix.userId}> は無事でした。`) },
  ]);
  return update(pullFrame(duel, state, em, ix.userId, ''));
}

/** 引き金を引いている最中の演出。 */
function pullFrame(duel, state, em, userId, tail) {
  return {
    content: '',
    embeds: [
      embed({
...
function resultPayload(duel, state, settings, em, winner, pot) {
