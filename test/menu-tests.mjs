...
await test('別の選択肢には移せないが、同じ選択肢には上乗せできる', async () => {
  const pollId = globalThis.__pollId;
  const moved = await pressPoll(`pl:bet:${pollId}:1`, { userId: OTHER });
  assert.match(screenText(moved), /別の選択肢/);
  assert.equal((await pollLib.betOf(db, pollId, OTHER)).option_idx, 0, '賭けは動かない');

  const form = await pressPoll(`pl:bet:${pollId}:0`, { userId: OTHER });
  assert.equal(form.type, 9, '上乗せの入力フォームが開く');
  assert.match(JSON.stringify(form.data), /上乗せ/);

  await pressPoll(`pl:amount:${pollId}:0`, { type: 5, userId: OTHER, fields: { amount: '200' } });
  assert.equal(await eco.getBalance(db, GUILD, OTHER), 500);
  assert.equal((await pollLib.betOf(db, pollId, OTHER)).amount, 500, '賭け金が積み上がる');
  assert.equal((await pollLib.betsOf(db, pollId)).length, 1, '参加者は増えない');
});

await test('払えない額は上乗せできず、賭け金も動かない', async () => {
  const pollId = globalThis.__pollId;
  const payload = await pressPoll(`pl:amount:${pollId}:0`, {
    type: 5, userId: OTHER, fields: { amount: '999999' },
  });
  assert.match(screenText(payload), /残高が足りません/);
  assert.equal((await pollLib.betOf(db, pollId, OTHER)).amount, 500, '積んだぶんが戻る');
  assert.equal(await eco.getBalance(db, GUILD, OTHER), 500);
});
...
await test('参加費制の大会は上乗せできない', async () => {
  await press('m:poll:create', {
    type: 5,
    fields: { question: '参加費ありのお題', options: 'A\nB', minutes: '60', stake: '100' },
  });
  const pollId = (await db.get('SELECT * FROM polls ORDER BY id DESC')).id;

  await eco.setBalance(db, GUILD, OTHER, 1000, 'test');
  await pressPoll(`pl:bet:${pollId}:0`, { userId: OTHER });
  assert.equal((await pollLib.betOf(db, pollId, OTHER)).amount, 100);

  const payload = await pressPoll(`pl:bet:${pollId}:0`, { userId: OTHER });
  assert.match(screenText(payload), /上乗せはできません/);
  assert.equal(await eco.getBalance(db, GUILD, OTHER), 900, '二重に取られない');
});

await test('締切を空欄にすると出題者が締め切るまで受け付ける', async () => {
  await press('m:poll:create', {
    type: 5,
    fields: { question: '締切なしのお題', options: 'A\nB', minutes: '', stake: '' },
  });
  const poll = await db.get('SELECT * FROM polls ORDER BY id DESC');
  assert.equal(poll.open_ended, 1);

  const board = ctx.sent.at(-1).payload;
  assert.match(JSON.stringify(board.embeds[0].fields), /出題者が締め切るまで/);

  await db.run('UPDATE polls SET closes_at = ?2 WHERE id = ?1', poll.id, Date.now() - 1000);
  assert.equal(
    (await pollLib.duePolls(db)).some((row) => row.id === poll.id),
    false,
    '時間で勝手に締まらない',
  );

  const closed = await pressPoll(`pl:close:${poll.id}`, { userId: ME });
  assert.equal(closed.type, 7);
...
await test('あそぶ画面から新しいゲームを開ける', async () => {
  ...
  const ids = customIds(payload);
  for (const screen of ['dd', 'rr', 'cs', 'mine']) {
    assert.ok(ids.includes(`m:${screen}:open`), `${screen} のボタンがある`);
  }
  assertAllButtonsWork(payload, 'あそぶ');

  // 運命の扉は2人用として並べつつ、1人でも遊べる導線を残す
  const versus = await press('m:dd:open');
  assert.ok(customIds(versus).includes('m:doors:open'), '1人で遊ぶボタンがある');
});
...
await test('ためていないとシュートは選べない', async () => {
  const duel = await startDuelBetween('cs');
  const payload = await pressDuel(`d:move:${duel.id}:shoot`, { userId: ME });
  assert.match(screenText(payload), /エネルギーが足りません/);

  const unknown = await pressDuel(`d:move:${duel.id}:big`, { userId: ME });
  assert.match(screenText(unknown), /不明な手/, 'ビッグシュートは無くなった');
});

await test('2回撃ち抜けば決着し、勝った側が総取り', async () => {
  const duel = await startDuelBetween('cs');
  const round = async (mine, theirs) => {
    await pressDuel(`d:move:${duel.id}:${mine}`, { userId: ME });
    await pressDuel(`d:move:${duel.id}:${theirs}`, { userId: OTHER });
    return duelLib.stateOf(await db.get('SELECT * FROM duels WHERE id = ?1', duel.id));
  };

  await round('charge', 'charge');
  const hit = await round('shoot', 'charge');
  assert.deepEqual(hit.hp, { challenger: 2, opponent: 1 }, '1回目は当たっても終わらない');
  assert.equal((await db.get('SELECT status FROM duels WHERE id = ?1', duel.id)).status, 'playing');

  await round('charge', 'guard');
  await round('shoot', 'charge');

  const done = await db.get('SELECT * FROM duels WHERE id = ?1', duel.id);
  assert.equal(done.status, 'done', '2回当てたら決着');
  assert.equal(duelLib.stateOf(done).hp.opponent, 0);
...
await test('1人プレイ: 当たれば倍率が伸び、持ち帰れる', async () => {
  ...
  for (let i = 0; i < 40 && !won; i++) {
    ...
    const payload = await press('m:doors:pick:red');
    const after = await db.get('SELECT * FROM doors_games WHERE guild_id = ?1 AND user_id = ?2', GUILD, ME);
    if (after && after.steps > 0) {
      won = true;
      assert.match(JSON.stringify(ctx.animated.at(-1)), /あたり/, '結果が大きく出る');
    } else {
      assert.match(JSON.stringify(ctx.animated.at(-1)), /はずれ/, '外れも大きく出る');
    }
  }
  ...
  assert.equal(await eco.getBalance(db, GUILD, ME), balanceBefore + 200);
...
await test('2人プレイ: 山分けを選ぶと半分ずつ', async () => {
  ...
  for (let i = 0; i < 30 && state.steps === 0; i++) {
    ...
    const shown = JSON.stringify(ctx.animated.at(-1));
    assert.match(shown, /あたり|はずれ/, '結果が大きく出る');
  ...
  assert.equal(sharing.multiplier, 2 ** sharing.steps, '1枚ごとに2倍');
  const gained =
